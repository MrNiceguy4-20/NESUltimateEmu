## Phase 94 - Board compatibility hardening: FME-7, VRC6, and MMC5

This checkpoint expands board-level compatibility without changing the validated CPU/PPU/APU accuracy baseline. Mapper 69 (FME-7) now banks declared NES 2.0 PRG-RAM through register 8 instead of aliasing every `$6000-$7FFF` selection to the first 8 KiB. A 32 KiB WRAM image therefore exposes four distinct banks as expected.

VRC6 mapper 24/26 `$B003` handling now models the register as the chip's PPU-banking control rather than a two-bit mirroring shortcut. Pattern-table grouping, CHR-backed nametables, CIRAM A10 routing/override modes, and `$B003.7` PRG-RAM enable are represented; both supplied VRC6 board-test ROM variants reach `All Tests Passed!`.

MMC5 CHR selection now distinguishes the `$5120-$5127` and `$5128-$512B` register sets according to sprite mode instead of routing all background fetches through set B. MMC5 PRG-RAM decode is also physical-size aware: absent chip-select regions return open bus rather than modulo-aliasing into installed SRAM, and bank bit 3 is retained for the 128 KiB NES 2.0 configuration. Mapper probes cover these cases.

Headless testing gained `--dump-frame PATH` and optional `--press-start` to make visual/manual mapper ROMs reproducible without SDL or OCR. The VRC6 test suite requires a long execution window; captures made before roughly 22 million CPU cycles can legitimately remain blank while the tests are still running.

Validation for this checkpoint: built-in probes **13/13**, AccuracyCoin **141/141**, configured non-AccuracyCoin CTest corpus **33/33**, and ASan+UBSan built-in probes **13/13**. Save-state version is **72** because VRC6 `$B003` state and MMC5 CHR-selection state are now serialized.
...
## Submapper / N163 hardening checkpoint

- Mapper 28 (Action 53) PRG decoding is regression-checked exhaustively against the published reference formula across all 64 mode values, 256 outer-bank values, 16 inner-bank values, and both CPU A14 halves (524,288 comparisons).
- Namco 163 external WRAM write-enable now requires `$F800` bits 7-4 to equal `%0100` exactly. Values `$50`, `$60`, and `$70` no longer incorrectly enable writes merely because bit 6 is set; bits 0-3 continue to protect the four 2 KiB WRAM windows independently.
- AccuracyCoin remains 141/141 after these mapper changes.
