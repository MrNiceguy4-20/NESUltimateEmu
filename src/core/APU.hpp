...
private:
    ...
    struct Dmc {
        ...
        // Explicit-stop DMA bug: if $4015 disables playback during the APU
        // cycle immediately before a reload would schedule, a one-cycle DMA
        // halt is still attempted. A CPU write on that halt slot suppresses
        // the aborted DMA entirely.
        uint8_t explicitStopWindow = 0;
    ...
    Noise m_noise;
    Dmc m_dmc;
    // AccuracyCoin accepts two observed implicit-stop behaviors. Default to
    // the mid-1990-or-later NTSC CPU behavior used by later NES revisions.
