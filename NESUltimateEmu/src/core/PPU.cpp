...
uint8_t PPU::cpuRead(uint16_t addr)
{
    ...
    switch (addr & 0x7) {
    case 2:
        data = (m_status & 0xE0) | (m_busLatch & 0x1F);
        // A status read immediately before the VBlank-set dot suppresses the
        // VBlank flag (and therefore NMI) for this frame.
        if (m_scanline == 241 && m_cycle == 1)
    ...
    case 7:
    {
        ...
        if (vramAddr >= 0x3F00) {
            // Palette reads bypass the delayed PPUDATA buffer, but the
            // internal buffer is still refreshed from the mirrored
            // nametable address underneath palette space.
            data = static_cast<uint8_t>((m_busLatch & 0xC0) | (ppuRead(vramAddr) & 0x3F));
        ...
        // PPUDATA increments the internal VRAM address after the memory
        // access. That increment also changes the address presented by the
        // PPU bus. MMC3 observes A12 on this bus, so a CPU read at $0FFF
        // followed by the automatic increment to $1000 must generate the
        // same qualified A12 rising edge as a $2006 address change.
...
void PPU::cpuWrite(uint16_t addr, uint8_t data)
{
    ...
    // CPU writes drive the full PPU I/O bus latch.
    m_busLatch = data;
}
...
PPU::DebugState PPU::debugState() const
{
    DebugState state;
    state.scanline = m_scanline;
    state.cycle = m_cycle;
    state.oddFrame = m_oddFrame;
    state.ctrl = m_ctrl;
    state.mask = m_mask;
    ...
    state.t = m_t;
    state.fineX = m_x;
    state.writeToggle = m_w;
    state.dataBuffer = m_dataBuffer;
    state.spriteCount = m_spriteCount;
    state.busLatch = m_busLatch;
    state.nmiLine = m_nmiLine;
    state.nmiDelay = m_nmiDelay;
    state.masterClock = m_masterClock;
    return state;
}

uint8_t PPU::debugCpuRead(uint16_t addr) const
{
    switch (addr & 7) {
    case 0: return m_ctrl;
    case 1: return m_mask;
    case 2: return static_cast<uint8_t>((m_status & 0xE0) | (m_busLatch & 0x1F));
    case 3: return m_oamAddr;
    case 4: return m_oam[m_oamAddr];
    case 7: return m_dataBuffer;
    default: return m_busLatch;
    }
}

uint8_t PPU::debugVramRead(uint16_t addr) const
{
    // This debugger view intentionally does not read CHR through the cartridge,
    // because MMC2/MMC4 pattern reads change mapper latch state. Nametable and
    // palette VRAM can be inspected without any emulation side effects.
    addr &= 0x3FFF;
    if (addr >= 0x2000 && addr <= 0x3EFF) {
        NametableSource source = NametableSource::Ciram;
        uint32_t mapped = 0;
    ...
    if (addr >= 0x3F00) {
        ...
        if ((paletteAddr & 0x13) == 0x10) paletteAddr &= ~0x10;
        return m_palette[paletteAddr] & 0x3F;
    }
    return 0;
}
