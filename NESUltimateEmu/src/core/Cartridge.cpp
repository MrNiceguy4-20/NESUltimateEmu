...
void Cartridge::mapperDebugState(std::vector<uint8_t>& out) const
{
    out.clear();
    if (m_mapper)
        m_mapper->saveState(out);
}
...
bool Cartridge::cpuRead(uint16_t addr, uint8_t& data)
{
    ...
    uint8_t regData = 0;
    if (addr >= 0x4020 && m_mapper->cpuReadRegister(addr, regData, false)) {
        data = regData;
        return true;
    }
...
uint8_t Cartridge::debugCpuRead(uint16_t addr) const
{
    if (!m_loaded || !m_mapper) return 0;

    uint8_t regData = 0;
    if (addr >= 0x4020 && m_mapper->cpuReadRegister(addr, regData, true))
        return regData;

    uint32_t mapped = 0;
    if (addr >= 0x6000 && m_mapper->mapPrgRam(addr, mapped, false) && mapped < m_prgRam.size())
        return m_prgRam[mapped];
    if (addr < 0x6000) return 0;
    if (m_mapper->cpuMapRead(addr, mapped) && mapped < m_prgRom.size())
        return m_prgRom[mapped];
    return 0;
}
